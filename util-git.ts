...
export type ResolvedRepoRelFilepath = [relFilepath: string, commitsIfWasNotFoundInHead: null | string[]];

export function resolveRepoRelFilepath(pathSuffix: string, repoRelFilepaths: string[], sinceCommittish: string): ResolvedRepoRelFilepath {
	const matchingSuffixes: string[] = repoRelFilepaths.filter(x => x.endsWith(pathSuffix));

	if (matchingSuffixes.length === 1) {
		return [matchingSuffixes[0], null]
